...
namespace FusionShaderTools {
	...
	EShLanguage LanguageFromType(EShaderStageType stage)
	{
		if (stage == EShaderStageType::Vertex)		return EShLanguage::EShLangVertex;
		if (stage == EShaderStageType::Fragment)	return EShLanguage::EShLangFragment;
		if (stage == EShaderStageType::Geometry)	return EShLanguage::EShLangGeometry;
	...
	void SpirVCompiler::LoadSource(const fs::path& filePath, std::map<EShaderStageType, std::string>& outSourceMap) {
		std::string source;
		...
		while (pos != std::string::npos)
		{
			...
			EShaderStageType stageType = ShaderStageFromString(typeName);
			assert(stageType != EShaderStageType::None && "Invalid shader type specified");
	...
	glslang::TShader* SpirVCompiler::ParseStage(EShaderStageType stage, const std::string& source)
	{
		glslang::TShader* outShader = new glslang::TShader(LanguageFromType(stage));
		...
		outShader->setEnvInput(glslang::EShSourceGlsl, LanguageFromType(stage), Client, ClientInputSemanticsVersion);
